...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (emailLower) {
        try {
          ...
          if (!uSnap.empty) {
            matchedProfile = uSnap.docs[0].data() as UserProfile;
          }
        } catch (err) {
          console.error("fetchProfile query existing users error:", err);
        }
      }
      ...
      if (emailLower && !emailLower.endsWith('@suvenedu.demo')) {
        try {
          const sRef = collection(db, 'schools');
          const q = query(sRef, where('adminEmail', '==', emailLower));
          ...
          if (!snap.empty) {
            ...
            const allSchools = await getDocs(sRef);
          ...
          console.error("fetchProfile school verification error:", e);
        }
      }
      ...
      if (!docSnap.exists()) {
        ...
        let permissions: AppPermission[] = ['take_exams'];
        ...
        if (isSystemAdmin) {
          ...
          role = 'school';
          permissions = ['manage_exams', 'view_results', 'manage_students' as any];
          schoolId = realSchoolId || 'school-core-node-1';
        } else if (matchedProfile) {
          role = matchedProfile.role || 'student';
          permissions = (matchedProfile.permissions as AppPermission[]) || ['take_exams'];
        ...
        const newProfile: UserProfile = {
          ...
          name: matchedProfile?.name || firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'Anonymous',
        ...
        if (isSystemAdmin && currentProfile.role !== 'admin') {
  ...
  const signUpWithEmail = async (email: string, pass: string, name: string, role: 'admin' | 'school' | 'student', schoolId?: string) => {
    try {
      ...
      const permissions: AppPermission[] = role === 'admin' 
        ? ['manage_exams', 'view_results'] 
        : role === 'school' 
          ? ['manage_exams', 'view_results', 'manage_students' as any]
          : ['take_exams'];
