    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      if (schoolFiltered.length === 0 && !isSeeding && fetched.length === 0) {
        ...
        try {
          ...
          console.error("Failed to seed default syllabus:", error);
      ...
      setLoading(false);
      handleFirestoreError(error, OperationType.LIST, 'syllabus');
    });
    ...
    <div className="space-y-8 pb-20">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center p-32 space-y-4">
          ...
          <p className="text-slate-400 text-xs font-black uppercase tracking-wider">Compiling Syllabus Coordinates...</p>
        </div>
      ) : (
