...
export const AdminSchoolManagement: React.FC = () => {
  ...
  const handleRemoveSchool = async (schoolId: string) => {
    const confirmRemove = window.confirm("Are you sure you want to completely remove this school node? This action cannot be undone.");
    if (!confirmRemove) return;

    try {
      // Delete DB Query (Alternative: Soft delete can be done via updateDoc(docRef, { isDeleted: true }))
      await deleteDoc(doc(db, 'schools', schoolId));
      toast.success("School node removed from Direct Registry");
    } catch (error) {
      console.error(error);
    ...
    <div className="school-section">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div className="space-y-2">
          <Badge className="bg-indigo-50 text-indigo-600 border-indigo-100 px-3 py-1 font-bold text-[10px] uppercase tracking-widest mb-2">Platform Administration</Badge>
          <h2 className="text-4xl font-display font-black text-slate-900 tracking-tight leading-none">Global Onboarding</h2>
          <p className="text-slate-500 font-medium max-w-lg">Provision new institutional nodes, configure security policies, and manage global school accessibility.</p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 w-full lg:w-auto">
          <div className="relative flex-1 min-w-[280px]">
             ...
             <Input 
               placeholder="Search entities by name or admin..." 
               className="pl-11 h-12 border-slate-200 bg-white shadow-sm focus:ring-2 focus:ring-indigo-500/20"
               value={searchQuery}
          ...
          <div className="flex p-1 bg-slate-100 rounded-xl border border-slate-200 shadow-inner">
             <button 
               ...
               className={`p-2 rounded-lg transition-all ${view === 'grid' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
             >
               <LayoutGrid size={20} />
             </button>
             <button 
               onClick={() => setView('list')}
               className={`p-2 rounded-lg transition-all ${view === 'list' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
             >
               <ListIcon size={20} />
             </button>
          </div>
          ...
          <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
            <SheetTrigger render={
              <Button className="bg-indigo-600 hover:bg-slate-900 h-12 px-6 shadow-[0_4px_20px_-4px_rgba(79,70,229,0.4)] rounded-xl font-bold flex items-center gap-2 group transition-all">
                     ...
                     <div className="space-y-3">
                       ...
                       <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                         ...
                         <div className="flex gap-1.5">
                           ...
                           <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg" onClick={() => handleRemoveSchool(school.id)}>
                             <Trash size={14} />
                           </Button>
                         </div>
                       </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-100/80 border-b border-slate-200">
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">School / Node Identity</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Region / Location</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-center">Enrolled Students</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-center">Live Attendance Rate</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-center">Avg Examination Score</th>
                    ...
                    <tr key={school.id} className="hover:bg-indigo-50/10 transition-colors group font-mono text-xs animate-none">
                      <td className="px-8 py-4.5 font-sans">
                        <div className="flex items-center gap-4">
                          <div className="h-10 w-10 min-w-[40px] rounded-xl bg-slate-100 group-hover:bg-indigo-600 group-hover:text-white flex items-center justify-center text-slate-400 transition-all">
                            <Building2 size={20} />
                          </div>
                          <div className="min-w-0">
                            <p className="font-black text-slate-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tight truncate max-w-[200px]">{school.name}</p>
                            <p className="text-[10px] font-mono text-slate-400 uppercase tracking-tighter truncate max-w-[200px]">{school.adminEmail}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-4.5 font-sans">
                        <div className="flex items-center gap-1.5 text-slate-600">
                          ...
                          <span className="text-xs font-bold uppercase">{school.region || 'Central Zone'}</span>
                        </div>
                      </td>
                      <td className="px-8 py-4.5 text-center font-bold text-slate-800">
                        {school.totalStudents || 120}
                      </td>
                      <td className="px-8 py-4.5 text-center">
                        <div className="inline-flex items-center gap-2">
                          <div className="w-12 bg-slate-100 h-1.5 rounded-full overflow-hidden inline-block border border-slate-200">
                            <div className="bg-emerald-500 h-full" style={{ width: `${school.attendanceRate || 95}%` }} />
                          </div>
                          <span className="text-xs font-bold text-emerald-600">{school.attendanceRate || 97.4}%</span>
                        </div>
                      </td>
                      <td className="px-8 py-4.5 text-center">
                        <Badge className="bg-indigo-50 text-indigo-700 border-indigo-150 hover:bg-indigo-50 font-bold px-2 py-0.5 rounded-md text-xs">
                          {school.avgScore || 75.8} / 100
                        </Badge>
                      </td>
                      <td className="px-8 py-4.5 text-center">
                         <div className="flex items-center justify-center gap-3">
                           <div className={`h-2.5 w-2.5 rounded-full ${school.status === 'active' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                           <Switch 
                             checked={school.status === 'active'} 
                             onCheckedChange={() => toggleStatus(school.id, school.status)} 
                           />
                      ...
                      <td className="px-8 py-4.5 text-right font-sans">
                         <div className="flex items-center justify-end gap-2.5">
                           <Button 
                             ...
                             className="h-8 px-3 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold flex items-center gap-1 text-[11px]"
                           >
                             <Edit size={12} /> Edit
                           </Button>
                           <Button 
                             onClick={() => handleRemoveSchool(school.id)}
                             className="h-8 px-3 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold flex items-center gap-1 text-[11px]"
                           >
